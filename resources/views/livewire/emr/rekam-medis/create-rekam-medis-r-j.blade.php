<div class="fixed inset-0 z-40 flex items-center justify-center bg-gray-500/75" role="dialog" aria-modal="true"
    ...
    <div class="relative w-full max-w-6xl max-h-[95vh] overflow-y-auto bg-white rounded-lg shadow-xl">
        ...
        <div
            ...
            <h3 id="judul-rj" class="text-2xl font-semibold">{{ $myTitle }}</h3>
        ...
        <div class="h-full overflow-y-auto bg-white">
            ...
            <div class="px-4 pt-4">
                <div class="grid grid-cols-4 gap-3 p-3 border border-gray-900 rounded-lg print-avoid-break">
                    <div class="flex flex-col items-center justify-start col-span-1 text-center">
                            ...
                            <dl class="divide-y divide-gray-100/60">
                                ...
                                <div class="py-1">
                                    <div class="flex flex-col gap-1 md:flex-row md:gap-8">
                                        ...
                                        <div class="flex items-baseline flex-1 min-w-0 gap-2">
                                            <dt class="w-20 shrink-0 after:content-[':'] after:ml-1">NIK
                                            </dt>
                                            <dd
                                ...
                                <!-- ID BPJS • Klaim (dua kolom fleksibel) -->
                                <div class="py-1">
                                    <div class="flex flex-col gap-1 md:flex-row md:gap-8">
                                        <div class="flex items-baseline flex-1 min-w-0 gap-2">
                                            <dt class="w-32 shrink-0 after:content-[':'] after:ml-1">ID
                                                BPJS</dt>
                                            <dd
                                                class="flex-1 min-w-0 truncate print:whitespace-normal print:overflow-visible">
                                ...
                                <div class="flex items-baseline gap-2 py-1">
                                    ...
                                    <dd class="flex-1 min-w-0 truncate print:whitespace-normal print:overflow-visible">
            ...
            {{-- PENGKAJIAN PERAWAT --}}
            @php
                $toYes = fn($v) => (bool) ($v ?? false) ? 'Ya' : '-';
            @endphp
            <div class="px-4">
                <div class="border-b border-gray-900 rounded-b-md border-x print-avoid-break">
                    <div class="grid gap-0 md:grid-cols-4">
                        <div class="p-3 text-sm font-semibold uppercase bg-gray-100">
                            pengkajian perawat
                        </div>
                        <div class="p-3 text-sm leading-6 md:col-span-3">
                            <span class="font-semibold">Status Psikologis :</span>
                            {{ $toYes(data_get($dataDaftarTxn, 'anamnesa.statusPsikologis.tidakAdaKelainan')) === 'Ya' ? 'Tidak ada kelainan' : '-' }}
                            /
                            {{ $toYes(data_get($dataDaftarTxn, 'anamnesa.statusPsikologis.marah')) === 'Ya' ? 'Marah' : '-' }}
                            /
                            {{ $toYes(data_get($dataDaftarTxn, 'anamnesa.statusPsikologis.ccemas')) === 'Ya' ? 'Cemas' : '-' }}
                            /
                            {{ $toYes(data_get($dataDaftarTxn, 'anamnesa.statusPsikologis.takut')) === 'Ya' ? 'Takut' : '-' }}
                            /
                            {{ $toYes(data_get($dataDaftarTxn, 'anamnesa.statusPsikologis.sedih')) === 'Ya' ? 'Sedih' : '-' }}
                            /
                            {{ $toYes(data_get($dataDaftarTxn, 'anamnesa.statusPsikologis.cenderungBunuhDiri')) === 'Ya' ? 'Resiko Bunuh Diri' : '-' }}
                            /
                    ...
                    <div class="text-sm divide-y divide-gray-200">
                        ...
                        <div class="grid grid-cols-4 gap-2 p-3">
                            ...
                            <div class="col-span-2">
                                ...
                                /
                                {{ data_get($dataDaftarTxn, 'penilaian.resikoJatuh.skalaHumptyDumpty.skalaHumptyDumptyDesc') ?? '-' }}<br>
                                ...
                                <div class="overflow-hidden rounded-md ring-1 ring-gray-300">
                                    <div class="overflow-y-auto max-h-56">
                                        <table class="w-full table-fixed">
                                            <thead class="sticky top-0 text-xs uppercase bg-gray-50">
                                            ...
                                            <tbody class="text-sm divide-y divide-gray-200">
                                                @forelse (data_get($dataDaftarTxn,'anamnesa.rekonsiliasiObat',[]) as $rek)
                                                    <tr class="divide-x divide-gray-200">
                                                        <td class="px-2 py-1">{{ data_get($rek, 'namaObat', '-') }}
                                                        </td>
                ...
                <div class="border border-gray-900 md:col-span-1 rounded-r-md">
                    <div class="p-3 space-y-1 text-sm">
                        ...
                        <div class="text-center">
                            @php
                                $perawatCode = data_get(
                                    $dataDaftarTxn,
                                    'anamnesa.pengkajianPerawatan.perawatPenerimaCode',
                                );
                                $imgPerawat = optional(App\Models\User::where('myuser_code', $perawatCode)->first())
                                    ->myuser_ttd_image;
                            @endphp

                            @if ($imgPerawat)
                                <img class="h-24 mx-auto"
                                    src="{{ asset('storage/' . $imgPerawat) }}?v={{ \Illuminate\Support\Str::random(6) }}"
                                    alt="TTD Perawat" />
                            @else
                                <div
                                    class="flex items-center justify-center h-24 text-xs text-gray-400 border border-dashed rounded">
                                    (TTD Perawat belum tersedia)
                                </div>
                            @endif

                            <div class="mt-1 text-xs italic text-gray-500">ttd</div>
            ...
            <div class="px-4 mt-2">
                ...
                <div class="px-3 py-2 border-b border-gray-900 rounded-b-md border-x">
                    <div class="overflow-hidden rounded-md ring-1 ring-gray-300">
                        <table class="w-full table-fixed">
                            <thead class="sticky top-0 text-sm bg-gray-50">
                                <tr class="divide-x divide-gray-200">
                            ...
                            <tbody class="text-sm divide-y divide-gray-200">
                                <tr>
                                    <td class="px-2 py-2">{!! nl2br(e(data_get($dataDaftarTxn, 'diagnosisFreeText') ?? '-')) !!}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="px-3 py-2 border-b border-gray-900 rounded-b-md border-x">
                    <div class="overflow-hidden rounded-md ring-1 ring-gray-300">
                        <table class="w-full table-fixed">
                            ...
                            <tbody class="text-sm divide-y divide-gray-200">
                                <tr>
                                    <td class="px-2 py-2">{!! nl2br(e(data_get($dataDaftarTxn, 'procedureFreeText') ?? '-')) !!}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div class="px-3 py-2 text-sm border-b border-gray-900 rounded-b-md border-x">
                    ...
                    <div class="grid gap-4 md:grid-cols-4">
                        ...
                        <div class="flex flex-col justify-end text-center md:col-span-1">
                            ...
                            @php
                                $dokterCode = data_get($dataDaftarTxn, 'drId');
        ...
        <div class="sticky bottom-0 flex justify-end px-4 py-3 border-t bg-gray-50 no-print">
            <div wire:loading wire:target="cetakRekamMedisRJ"><x-loading /></div>
            <x-green-button :disabled="$disabledPropertyRjStatus" wire:click.prevent="cetakRekamMedisRJ()" wire:loading.remove>
                Cetak RM RJ
            </x-green-button>
        </div>
