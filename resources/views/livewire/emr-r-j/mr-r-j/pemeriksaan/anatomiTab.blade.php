                            ...
                            <div class="flex mt-2 ml-2">
                                @foreach ($pAnatomi['kelainanOptions'] as $kelainanOptions)
                                    ...
                                    <x-radio-button :label="__($kelainanOptions['kelainan'])" value="{{ $kelainanOptions['kelainan'] }}"
                                        wire:model="dataDaftarPoliRJ.pemeriksaan.anatomi.{{ $key }}.kelainan" />
                                @endforeach
                            </div>
                            <x-text-input-area id="dataDaftarPoliRJ.pemeriksaan.anatomi.{{ $key }}.kelainan"
                                placeholder="{{ strtoupper($key) }}" class="mt-1 ml-2" :errorshas="__($errors->has('dataDaftarPoliRJ.pemeriksaan.anatomi'))"
                                :disabled=$disabledPropertyRjStatus
                                wire:model.debounce.500ms="dataDaftarPoliRJ.pemeriksaan.anatomi.{{ $key }}.desc"
                                :rows="__('10')" />
