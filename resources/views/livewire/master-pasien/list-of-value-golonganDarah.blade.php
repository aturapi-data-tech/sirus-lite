<div x-data @click.outside="$wire.golonganDarahLovStatus = false" class="absolute">

    @if ($golonganDarahLovStatus)
        <!-- Dropdown list Title -->
        <div x-data x-init="$refs.golonganDarahLovSearch.focus()"
            ...
            <x-text-input x-ref="golonganDarahLovSearch" placeholder="Cari Data" class="mt-1 ml-2"
                wire:model="golonganDarahLovSearch" />
        </div>
        ...
        <div class="z-10 w-full bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <ul class="h-auto px-3 pt-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                @foreach ($golonganDarahLov as $lov)
                    <li wire:key='golonganDarahLov{{ $lov['golonganDarahId'] }}'>
                        <x-dropdown-link
                            wire:click="setMygolonganDarahLov('{{ $lov['golonganDarahId'] }}','{{ $lov['golonganDarahDesc'] }}')">
                            {{ $lov['golonganDarahId'] . '. ' . $lov['golonganDarahDesc'] }}
                        </x-dropdown-link>
                    </li>
                @endforeach
                @if (strlen($golonganDarahLovSearch) < 3 && count($golonganDarahLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                            {{ 'Masukkan minimal 3 karakter' }}
                        </div>
                    </li>
                @elseif(strlen($golonganDarahLovSearch) >= 3 && count($golonganDarahLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
