<div class="absolute">
    @if ($statusLovStatus)
        ...
        <div x-data x-init="$refs.statusLovSearch.focus()"
            ...
            <x-text-input x-ref="statusLovSearch" placeholder="Cari Data" class="mt-1 ml-2"
                wire:model="statusLovSearch" />
        </div>
        ...
        <div class="z-10 w-full bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <ul class="h-auto px-3 pt-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                @foreach ($statusLov as $lov)
                    <li wire:key='statusLov{{ $lov['statusId'] }}'>
                        <x-dropdown-link
                            wire:click="setMystatusLov('{{ $lov['statusId'] }}','{{ $lov['statusDesc'] }}')">
                            {{ $lov['statusId'] . '. ' . $lov['statusDesc'] }}
                        </x-dropdown-link>
                    </li>
                @endforeach
                @if (strlen($statusLovSearch) < 3 && count($statusLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                            {{ 'Masukkan minimal 3 karakter' }}
                        </div>
                    </li>
                @elseif(strlen($statusLovSearch) >= 3 && count($statusLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
