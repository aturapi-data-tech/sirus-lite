<div x-data @click.outside="$wire.pekerjaanLovStatus = false" class="absolute">

    @if ($pekerjaanLovStatus)
        <!-- Dropdown list Title -->
        <div x-data x-init="$refs.pekerjaanLovSearch.focus()"
            ...
            <x-text-input x-ref="pekerjaanLovSearch" placeholder="Cari Data" class="mt-1 ml-2"
                wire:model="pekerjaanLovSearch" />
        </div>
        ...
        <div class="z-10 w-full bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <ul class="h-auto px-3 pt-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                @foreach ($pekerjaanLov as $lov)
                    <li wire:key='pekerjaanLov{{ $lov['pekerjaanId'] }}'>
                        <x-dropdown-link
                            wire:click="setMypekerjaanLov('{{ $lov['pekerjaanId'] }}','{{ $lov['pekerjaanDesc'] }}')">
                            {{ $lov['pekerjaanId'] . '. ' . $lov['pekerjaanDesc'] }}
                        </x-dropdown-link>
                    </li>
                @endforeach
                @if (strlen($pekerjaanLovSearch) < 3 && count($pekerjaanLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                            {{ 'Masukkan minimal 3 karakter' }}
                        </div>
                    </li>
                @elseif(strlen($pekerjaanLovSearch) >= 3 && count($pekerjaanLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
