<div x-data @click.outside="$wire.agamaLovStatus = false" class="absolute">
    @if ($agamaLovStatus)
        ...
        <div x-data x-init="$refs.agamaLovSearch.focus()"
            ...
            <x-text-input x-ref="agamaLovSearch" placeholder="Cari Data" class="mt-1 ml-2"
                wire:model="agamaLovSearch" />
        </div>
        ...
        <div class="z-10 w-full bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <ul class="h-auto px-3 pt-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                @foreach ($agamaLov as $lov)
                    <li wire:key='agamaLov{{ $lov['agamaId'] }}'>
                        <x-dropdown-link wire:click="setMyagamaLov('{{ $lov['agamaId'] }}','{{ $lov['agamaDesc'] }}')">
                            {{ $lov['agamaId'] . '. ' . $lov['agamaDesc'] }}
                        </x-dropdown-link>
                    </li>
                @endforeach
                @if (strlen($agamaLovSearch) < 3 && count($agamaLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                            {{ 'Masukkan minimal 3 karakter' }}
                        </div>
                    </li>
                @elseif(strlen($agamaLovSearch) >= 3 && count($agamaLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
