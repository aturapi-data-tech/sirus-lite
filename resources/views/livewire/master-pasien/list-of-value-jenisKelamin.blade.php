<div x-data @click.outside="$wire.jenisKelaminLovStatus = false" class="absolute">

    @if ($jenisKelaminLovStatus)
        <!-- Dropdown list Title -->
        <div x-data x-init="$refs.jenisKelaminLovSearch.focus()"
            ...
            <x-text-input x-ref="jenisKelaminLovSearch" placeholder="Cari Data" class="mt-1 ml-2"
                wire:model="jenisKelaminLovSearch" />
        </div>
        ...
        <div class="z-10 w-full bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <ul class="h-auto px-3 pt-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                @foreach ($jenisKelaminLov as $lov)
                    <li wire:key='jenisKelaminLov{{ $lov['jenisKelaminId'] }}'>
                        <x-dropdown-link
                            wire:click="setMyjenisKelaminLov('{{ $lov['jenisKelaminId'] }}','{{ $lov['jenisKelaminDesc'] }}')">
                            {{ $lov['jenisKelaminId'] . '. ' . $lov['jenisKelaminDesc'] }}
                        </x-dropdown-link>
                    </li>
                @endforeach
                @if (strlen($jenisKelaminLovSearch) < 3 && count($jenisKelaminLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                            {{ 'Masukkan minimal 3 karakter' }}
                        </div>
                    </li>
                @elseif(strlen($jenisKelaminLovSearch) >= 3 && count($jenisKelaminLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
