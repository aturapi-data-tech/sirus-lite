<div x-data @click.outside="$wire.pendidikanLovStatus = false" class="absolute">

    @if ($pendidikanLovStatus)
        <!-- Dropdown list Title -->
        <div x-data x-init="$refs.pendidikanLovSearch.focus()"
            ...
            <x-text-input x-ref="pendidikanLovSearch" placeholder="Cari Data" class="mt-1 ml-2"
                wire:model="pendidikanLovSearch" />
        </div>
        ...
        <div class="z-10 w-full bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <ul class="h-auto px-3 pt-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                @foreach ($pendidikanLov as $lov)
                    <li wire:key='pendidikanLov{{ $lov['pendidikanId'] }}'>
                        <x-dropdown-link
                            wire:click="setMypendidikanLov('{{ $lov['pendidikanId'] }}','{{ $lov['pendidikanDesc'] }}')">
                            {{ $lov['pendidikanId'] . '. ' . $lov['pendidikanDesc'] }}
                        </x-dropdown-link>
                    </li>
                @endforeach
                @if (strlen($pendidikanLovSearch) < 3 && count($pendidikanLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                            {{ 'Masukkan minimal 3 karakter' }}
                        </div>
                    </li>
                @elseif(strlen($pendidikanLovSearch) >= 3 && count($pendidikanLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
