    ...
    <div class="fixed inset-0 z-10 overflow-y-auto">
        <div class="flex items-end justify-center min-h-full p-4 text-center sm:items-center sm:p-0">
            <div
                ...
                <!-- Selected Items -->
                @if (count($isPemeriksaanLaboratoriumSelected) > 0)
                    <div class="px-6">
                            ...
                            @foreach ($isPemeriksaanLaboratoriumSelected as $key => $isPemeriksaanLab)
                                <div
                                    ...
                                    <div class="flex-1 block">
                                        <div class="w-auto text-sm font-medium">{{ $isPemeriksaanLab['clabitem_desc'] }}
                                        </div>
                                        <div class="w-auto text-xs text-gray-600">Rp
                                            {{ number_format($isPemeriksaanLab['price']) }}</div>
                                    </div>
                                    <button type="button"
                                        ...
                                        wire:click.prevent="toggleLaboratorium({{ array_search($isPemeriksaanLab, $isPemeriksaanLaboratorium) }})">
                ...
                <!-- Available Items -->
                <div class="p-6">
                    <x-input-label value="Daftar Pemeriksaan Laboratorium" class="mb-4 text-lg font-semibold" />

                    <div class="flex flex-wrap -mx-2">
                        @foreach ($isPemeriksaanLaboratorium as $key => $isPemeriksaanLab)
                            @php
                                $bgCardPropertyColor = $isPemeriksaanLab['labStatus']
                                    ? 'bg-green-100 border-green-300'
                                    : 'bg-white border-gray-200';
                                $textColor = $isPemeriksaanLab['labStatus'] ? 'text-green-800' : 'text-gray-900';
                            @endphp
                            <div class="w-full px-2 mb-4 sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5">
                                <div wire:click.prevent="toggleLaboratorium({{ $key }})"
                                    ...
                                    <div class="flex flex-col items-center text-center">
                                        ...
                                        <span class="text-xs font-medium text-gray-600">
                                            Rp {{ number_format($isPemeriksaanLab['price']) }}
                                        </span>
                                        @if ($isPemeriksaanLab['labStatus'])
                    ...
                    @if (empty($isPemeriksaanLaboratorium))
                        <div class="py-8 text-center">
                            ...
                            <p class="mt-2 text-gray-500">Tidak ada data pemeriksaan laboratorium tersedia.</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="sticky bottom-0 flex justify-between px-6 py-4 border-t bg-gray-50">
                    <div class="text-sm text-gray-600">
                        @if (count($isPemeriksaanLaboratoriumSelected) > 0)
                            <span class="font-semibold">{{ count($isPemeriksaanLaboratoriumSelected) }} item
                                terpilih</span>
                        @else
                    ...
                    <div class="flex space-x-2">
                        <x-alternative-button wire:click="closeModalLaboratorium()" type="button">
                            Batal
                        </x-alternative-button>
                        <div wire:loading wire:target="kirimLaboratorium">
                            <x-loading />
                        </div>
                        <x-green-button :disabled="empty($isPemeriksaanLaboratoriumSelected)" wire:click.prevent="kirimLaboratorium()" type="button"
                            wire:loading.remove>
                            Kirim Laboratorium
                        </x-green-button>
                    </div>
