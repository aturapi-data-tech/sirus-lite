<div class="absolute z-50">
    @if ($hfisLovStatus)
        <!-- Dropdown list Title -->
        <div x-data x-init="$refs.hfisLovSearch.focus()"
            ...
            {{-- <x-text-input x-ref="hfisLovSearch" placeholder="Cari Data" class="mt-1 ml-2" wire:model="hfisLovSearch" /> --}}

            <div wire:loading wire:target="setMyhfisLov">
                <x-loading />
            </div>
        </div>
        ...
        <!-- Dropdown menu -->
        <div class="w-full overflow-auto bg-white rounded-lg shadow max-h-56 dark:bg-gray-700">
            <!-- Dropdown list -->



            <ul class="h-auto px-3 pt-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                @foreach ($hfisLov as $lov)
                    <li wire:key='hfisLov{{ $lov['kode'] }}'>
                        <x-dropdown-link wire:click="setMyhfisLov('{{ $lov['kode'] }}','{{ $lov['nama'] }}')">
                            {{ $lov['kode'] . '. ' . $lov['nama'] }}
                        </x-dropdown-link>
                    </li>
                @endforeach
                @if (strlen($hfisLovSearch) < 3 && count($hfisLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                            {{ 'Masukkan minimal 3 karakter' }}
                        </div>
                    </li>
                @elseif(strlen($hfisLovSearch) >= 3 && count($hfisLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
