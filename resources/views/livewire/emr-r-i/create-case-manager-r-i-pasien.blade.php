            ...
            <div class="absolute overflow-auto bg-white rounded-t-lg inset-4">
                ...
                <div class="">
                    <livewire:emr-r-i.display-pasien.display-pasien
                        :wire:key="$regNoRef.'display-pasien-case-manager-pasien'" :riHdrNoRef="$riHdrNoRef">
                </div>


                {{-- Transasi EMR --}}
                <div id="case-manager-pasien-TransaksiEMR" x-data="{ activeTabCaseManagerRI: @entangle('activeTabCaseManagerRI') }" class="grid grid-cols-1">
                    ...
                    <div class="px-2 mb-0 overflow-auto border-b border-gray-200">
                        <ul
                            ...
                            @foreach ($EmrMenuCaseManagerRI as $EmrM)
                                <li wire:key="tab-{{ $EmrM['ermMenuId'] }}" class="mx-1 mr-0 rounded-t-lg "
                                    x-bind:class="activeTabCaseManagerRI
                                    === '{{ $EmrM['ermMenuId'] }}' ?
                                        'text-primary border-primary bg-gray-100' :
                                        'border border-gray-200'">
                                    <label
                                        class="inline-block p-2 border-b-2 border-transparent rounded-t-lg hover:text-gray-600 hover:border-gray-300"
                                        x-on:click="activeTabCaseManagerRI ='{{ $EmrM['ermMenuId'] }}'"
                                        wire:click="$set('activeTabCaseManagerRI', '{{ $EmrM['ermMenuId'] }}')">{{ $EmrM['ermMenuName'] }}</label>
                                </li>
                            @endforeach
                    ...
                    <div class="w-full mx-2 mr-2 rounded-lg bg-gray-50 "
                        :class="{
                            'active': activeTabCaseManagerRI === 'caseManagerRI'
                        }"
                        x-show.transition.in.opacity.duration.600="activeTabCaseManagerRI === 'caseManagerRI'">
                        <livewire:emr-r-i.case-manager-r-i.case-manager-r-i
                            :wire:key="'content-caseManagerRI'.$riHdrNoRef" :riHdrNoRef="$riHdrNoRef" :regNoRef="$regNoRef">
                    </div>
