    ...
    <div class="w-full mb-1">
        <table class="w-full text-sm text-left text-gray-900 table-auto dark:text-gray-400">
            ...
            <tbody class="bg-white dark:bg-gray-800">
                @php
                    ...
                    $sortedPemakaianOksigen = collect(
                        ...
                        ->sortByDesc(function ($item) {
                            $tgl = $item['tanggalWaktuMulai'] ?? '';
                            if (!$tgl) {
                                return 0; // kosong, jadikan paling bawah
                            }
                            try {
                                return Carbon::createFromFormat('d/m/Y H:i:s', $tgl, env('APP_TIMEZONE'))->timestamp;
                            } catch (\Exception $e) {
                ...
                @if ($sortedPemakaianOksigen->isNotEmpty())
                    @foreach ($sortedPemakaianOksigen as $key => $pemakaian)
                        <tr class="border-b group dark:border-gray-700">
                            ...
                            <td class="text-center group-hover:bg-gray-100 group-hover:text-primary">
                                ...
                                Model :{{ $pemakaian['modelPenggunaan'] }}
                                </br>
                                Jenis Alat :{{ $pemakaian['jenisAlatOksigen'] }}
                                @if ($pemakaian['jenisAlatOksigen'] === 'Lainnya')
                                    <br> <span
                                        class="text-xs text-gray-500">({{ $pemakaian['jenisAlatOksigenDetail'] }})</span>
                                @endif
                                </br>
                                Durasi :{{ $pemakaian['durasiPenggunaan'] }}
                            </td>
                            <td class="text-center group-hover:bg-gray-100 group-hover:text-primary">
                                {{ $pemakaian['dosisOksigen'] }}
                                @if ($pemakaian['dosisOksigen'] === 'Lainnya')
                                    <br> <span
                                        class="text-xs text-gray-500">({{ $pemakaian['dosisOksigenDetail'] }})</span>
                                @endif
                            </td>
                            <td class="text-center group-hover:bg-gray-100 group-hover:text-primary">
