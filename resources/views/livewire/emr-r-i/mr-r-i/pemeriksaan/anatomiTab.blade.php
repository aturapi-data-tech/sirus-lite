<div>
    <div class="w-full mb-1">
        ...
        <div class="pt-0">
            ...
            <x-input-label for="dataDaftarRi.pemeriksaan.anatomi" :value="__('Anatomi')" :required="__(false)"
                class="pt-2 sm:text-xl" />

            <div id="TransaksiRawatJalan" x-data="{ activeTab: 'kepala' }" class="flex">
                <div class="w-[200px] h-80 overflow-auto">
                    @foreach ($dataDaftarRi['pemeriksaan']['anatomi'] as $key => $pAnatomi)
                ...
                <div class="w-full">
                    @foreach ($dataDaftarRi['pemeriksaan']['anatomi'] as $key => $pAnatomi)
                        <div class="p-4 rounded-lg bg-gray-50 dark:bg-gray-800"
                            ...
                            <x-input-label for="dataDaftarRi.pemeriksaan.anatomi.{{ $key }}.kelainan"
                            ...
                            <div class="flex mt-2 ml-2">
                                @foreach ($pAnatomi['kelainanOptions'] as $kelainanOptions)
                                    ...
                                    <x-radio-button :label="__($kelainanOptions['kelainan'])" value="{{ $kelainanOptions['kelainan'] }}"
                                        wire:model="dataDaftarRi.pemeriksaan.anatomi.{{ $key }}.kelainan" />
                                @endforeach
                            </div>
                            <x-text-input-area id="dataDaftarRi.pemeriksaan.anatomi.{{ $key }}.kelainan"
                                placeholder="{{ strtoupper($key) }}" class="mt-1 ml-2" :errorshas="__($errors->has('dataDaftarRi.pemeriksaan.anatomi'))"
                                :disabled=$disabledPropertyRjStatus
                                wire:model.debounce.500ms="dataDaftarRi.pemeriksaan.anatomi.{{ $key }}.desc"
                                :rows="__('10')" />
