<div x-data @click.outside="$wire.dataDokterLovStatus = false" class="absolute">
    @if ($dataDokterLovStatus)
        <!-- Dropdown list Title -->
        <div x-data x-init="$refs.dataDokterLovSearch.focus()"
            ...
            <x-text-input x-ref="dataDokterLovSearch" placeholder="Cari Data" class="mt-1 ml-2"
                wire:model="dataDokterLovSearch" />
        </div>
        ...
        <div class="z-10 w-full bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <ul class="h-auto px-3 pt-3 pb-3 overflow-y-auto text-sm text-gray-700 dark:text-gray-200">
                @foreach ($dataDokterLov as $lov)
                    <li wire:key='dataDokterLov{{ $lov['dr_id'] }}'>
                        <x-dropdown-link wire:click="setMydataDokterLov('{{ $lov['dr_id'] }}','{{ $lov['dr_name'] }}')">
                ...
                @endforeach
                @if (strlen($dataDokterLovSearch) < 3 && count($dataDokterLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
                            {{ 'Masukkan minimal 3 karakter' }}
                        </div>
                    </li>
                @elseif(strlen($dataDokterLovSearch) >= 3 && count($dataDokterLov) == 0)
                    <li>
                        <div class="w-full p-4 text-sm text-center text-gray-500 dark:text-gray-400">
