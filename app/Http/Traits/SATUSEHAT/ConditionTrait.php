...
trait ConditionTrait
{
    ...
    /**
     * Membuat riwayat penyakit pribadi
     *
     * @param array $data
     *   - patientId, encounterId, icd10_code, icd10_display, disease_name,
     *     onsetDate, severity_code, severity_display, note, isPast(boolean)
     * @return array
     */
    public function createPersonalDisease(array $data): array
    {
        $payload = [
            'resourceType'    => 'Condition',
            'category'        => [[
            ...
            'clinicalStatus'  => [
                'coding' => [[
                    'system'  => 'http://terminology.hl7.org/CodeSystem/condition-clinical',
                    'code'    => $data['isPast'] ? 'inactive' : 'active',
                    'display' => $data['isPast'] ? 'Inactive' : 'Active',
                ]]
            ],
            'severity'        => [[
                'coding' => [[
                    'system'  => 'http://snomed.info/sct',
                    'code'    => $data['severity_code'] ?? '255604002',
                    'display' => $data['severity_display'] ?? 'Moderate',
                ]]
            ]],
            'note'            => [['text' => $data['note'] ?? '']],
        ];
    ...
    /**
     * Mencari riwayat penyakit berdasarkan pasien dan status
     *
     * @param string $patientId
     * @param bool   $past       // false=aktif, true=terdahulu
     * @return array
     */
    public function searchDiseases(string $patientId, bool $past = false): array
    {
        $status   = $past ? 'inactive' : 'active';
        $endpoint = "Condition?subject=Patient/{$patientId}";
        $endpoint .= "&clinical-status={$status}";
        $endpoint .= "&category=problem-list-item";
        return $this->makeRequest('get', $endpoint);
    }
