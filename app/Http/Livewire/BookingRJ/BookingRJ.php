    ...
    public function render()
    {
        ...
        $query = DB::table('referensi_mobilejkn_bpjs')
            ->select(
                ...
                'nohp',
                'kodepoli',
                DB::raw("(select distinct poli_desc from rsmst_polis where kd_poli_bpjs=referensi_mobilejkn_bpjs.kodepoli)poli_desc"),
                'pasienbaru',
                'norm',
                'kodedokter',
                DB::raw("(select distinct dr_name from rsmst_doctors where kd_dr_bpjs=referensi_mobilejkn_bpjs.kodedokter)dr_name and rownum = 1"),
