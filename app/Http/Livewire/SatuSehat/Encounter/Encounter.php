...
namespace App\Http\Livewire\SatuSehat\Encounter;
...
class Encounter
{
    ...
    {
        $identifier['system'] = 'http://sys-ids.kemkes.go.id/encounter/' . env('SATUSEHAT_ORGANIZATION_ID');
    ...
    {
        $this->encounter['serviceProvider']['reference'] = 'Organization/' .  env('SATUSEHAT_ORGANIZATION_ID');
    }

    public function addDiagnosis($id, $code, $display = null, $bundle = false)
    {
        // Look in database if display is null
        $code_check = DB::table('rsmst_mstdiags ')->where('icdx', $code)->first();
